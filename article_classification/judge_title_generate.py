...
with open("title_example.txt", "r", encoding="utf-8") as t_example:
    title_example = t_example.read()
...
def add_article(doc, answer):
    ...
    # 实时保存
    doc.save("./result/liangziwei_judgeBYtitle.docx")
        ...
        if len(headers) == 4 and len(data) == 4:
            ...
            check_response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"""这是一些已经做好判断的的例子：{title_example}。请判断以下内容是否符合“以国内外企业为主体的科技成果的发布，这些技术成果目前已经或者即将产生经济效益”的标准。只需回答“是”或“否”，不要添加任何解释：
            内容如下：
            {row['title']}"""}
                ],
                temperature=0.0
